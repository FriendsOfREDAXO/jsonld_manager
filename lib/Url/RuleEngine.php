...
class RuleEngine
{
     ...
     */
    public static function matchCurrentUrl()
    {
     ...
     */
    private static function getUrlRules($articleId)
    {
        ...
        if (self::$rulesCache === null || !isset(self::$rulesCache[$cacheKey])) {
            ...
            $rules = [];
            while ($sql->hasNext()) {
                $config = json_decode($sql->getValue('schema_config'), true) ?: [];
                $getParams = json_decode($sql->getValue('get_params'), true) ?: [];
     ...
     */
    private static function matchRule($rule, $currentUrl, $getParams)
    {
     ...
     */
    private static function validateParamValue($value, $validation)
    {
        ...
        if (!empty($validation['pattern'])) {
            return preg_match($validation['pattern'], $value);
        }
     ...
     */
    private static function extractRuleData($rule, $getParams)
    {
        $data = [];
     ...
     * @return array Array von URL-Regeln
     */
    public static function getArticleRules($articleId)
    {
        $sql = \rex_sql::factory();
        $sql->setQuery('
            SELECT * FROM '.\rex::getTable('jsonld_url_rules').' 
        ...
        while ($sql->hasNext()) {
            $rules[] = [
                ...
                'name' => $sql->getValue('name'),
                'url_pattern' => $sql->getValue('url_pattern'),
                'get_params' => json_decode($sql->getValue('get_params'), true),
                'schema_config' => json_decode($sql->getValue('schema_config'), true),
     ...
     */
    public static function validateRule($ruleData)
    {
        $errors = [];
     ...
     */
    public static function testRule($ruleData, $testUrl, $testParams = [])
    {
        $result = [
            'match' => false,
            'errors' => [],
